            ...
            child: BlocBuilder<ConvertPageCubit, ConvertPageState>(
              builder: (context, state) {
                return switch (state) {
                  ...
                  ConvertPageLoaded() => BlocListener<RatesCubit, RatesState>(
                    listener: (context, ratesState) {
                      if (ratesState is RatesLoaded) {
                        context.read<ConvertPageCubit>().updateAmount(
                          amount: state.amount,
                          fromCurrency: ratesState.rates.firstWhere(
                            (c) => c.symbol == state.fromCurrency.symbol,
                          ),
                          toCurrency: ratesState.rates.firstWhere(
                            (c) => c.symbol == state.toCurrency.symbol,
                          ),
                        );
                      }
                    },
